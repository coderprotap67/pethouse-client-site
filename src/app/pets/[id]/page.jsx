...
export default function PetDetailsPage() {
  ...
  const handleAdoptSubmit = async (e) => {
    ...
    const requestData = {
      ...
      requesterName: user.displayName || user.name || 'Anonymous User',
        ...
        <div className="lg:col-span-2 bg-[#1C2541]/90 backdrop-blur-md rounded-3xl overflow-hidden shadow-2xl border border-[#3A506B]/40">
          ...
          <div className="p-6 space-y-5">
            <div className="flex justify-between items-center gap-4">
              ...
              <span
                className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border backdrop-blur-sm ${
                  pet.status === 'adopted'
          ...
          {pet.status === 'adopted' ? (
            <div className="bg-red-950/20 border border-red-900/30 rounded-2xl p-4 text-center">
              <p className="text-red-400 font-bold text-sm">
                This pet has already been successfully adopted! 🎉
              </p>
            </div>
          ) : (
            <form onSubmit={handleAdoptSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  value={pet.name || ''}
                  readOnly
                  className="w-full px-4 py-2.5 bg-[#0B132B]/60 border border-[#3A506B]/50 text-slate-400 rounded-xl outline-none cursor-not-allowed text-sm font-medium"
                />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1">
